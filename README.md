...
Η τιμή `85` είναι το adaptive maximum των θετικών rule-based κριτηρίων πριν από bonuses/penalties. Η προθεσμία και η ακύρωση δεν αλλάζουν τη σχετικότητα.
...
Σε διαγωνισμούς με πολλαπλά CPV εφαρμόζεται ένας απλός, ήπιος coverage factor:

```text
coverage_factor = 1.00  όταν ταιριάζουν όλα τα CPV ή υπάρχει μόνο ένα
coverage_factor = 0.90  όταν ταιριάζει μέρος των CPV
```

Το `1/2` και το `1/16` λαμβάνουν την ίδια μικρή μείωση, επειδή ένα μόνο σχετικό CPV μπορεί να αντιστοιχεί σε lot στο οποίο μπορεί να συμμετάσχει η επιχείρηση. Κάθε πραγματικό CPV match έχει τελικό κατώφλι σχετικότητας `55`, ώστε να παραμένει ορατό για ανθρώπινο έλεγχο. Αν υπάρχει exact CPV του προφίλ, το τελικό score έχει κατώφλι `75` και χαρακτηρίζεται υψηλής προτεραιότητας ακόμη και μέσα σε multi-CPV tender. Ο λόγος matched/total εμφανίζεται στις αιτιολογίες ως πληροφορία και όχι ως λόγος απόρριψης. Αν δεν βρεθεί κανένα CPV που να ταιριάζει με το προφίλ, εφαρμόζεται penalty `-12`.

Τα πολύ γενικά parent CPV, όπως `33000000-0`, χρησιμοποιούνται κυρίως για broad monitoring και discovery. Τα descendant/family matches παραμένουν σε επίπεδο review χωρίς επιπλέον ενδείξεις από budget, περιοχή, PDF ή άλλα στοιχεία. Το ίδιο το exact επιλεγμένο CPV παραμένει υψηλής προτεραιότητας.
